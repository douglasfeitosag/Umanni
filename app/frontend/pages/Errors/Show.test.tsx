...
describe('delivery error page', () => {
  it('offers a focused, safe return without replaying the failed request', () => {
    render(<ErrorPage status={500} returnPath="/sign-in" />)
    ...
    expect(screen.getByRole('link', { name: 'Voltar ao início' })).toHaveAttribute('href', '/sign-in')
  ...
  it.each([
    ...
    ['/profile', '/profile'],
    ['/sign-in', '/sign-in'],
  ])('uses the supplied safe destination %s', (returnPath, expectedPath) => {
    render(<ErrorPage status={500} returnPath={returnPath} />)

    expect(screen.getByRole('link', { name: 'Voltar ao início' })).toHaveAttribute('href', expectedPath)
  })
})
