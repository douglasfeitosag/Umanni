...
export default function AdminUserForm({ user, roleOptions, canChangeRole = true }: { user?: UserView; roleOptions: RoleOption[]; canChangeRole?: boolean }) {
  const creating = !user
  ...
  return (
    <form onSubmit={submit} noValidate>
      ...
      <div className="field"><label htmlFor="role">Papel</label>{canChangeRole ? <select id="role" name="role" value={form.data.role} onChange={event => form.setData('role', event.target.value as Role)} aria-invalid={form.errors.role ? true : undefined} aria-describedby={form.errors.role ? 'role-error' : undefined}>{roleOptions.map(option => <option key={option.value} value={option.value}>{option.label}</option>)}</select> : <input id="role" value={user?.role === 'admin' ? 'Administrador' : 'Usuário regular'} readOnly />}{form.errors.role && <p className="field-error" id="role-error">{form.errors.role}</p>}</div>
