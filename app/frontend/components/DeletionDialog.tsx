...
export default function DeletionDialog({ action, label }: { action: string; label: string }) {
  ...
  return (
    <>
      ...
      <dialog ref={dialog} onClose={close} aria-labelledby="delete-title">
        {open && <form onSubmit={event => {
          ...
          <Field label="Confirmação" name="confirmation" value={form.data.confirmation} onChange={event => form.setData('confirmation', event.target.value)} error={form.errors.confirmation} autoFocus />
