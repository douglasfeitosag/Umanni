...
- Spec, plano e tarefas recebem revisão independente antes da execução; código, testes, documentação e evidências recebem nova revisão depois. Cada achado vai em comentário individual no PR, com ID, evidência, impacto, autor/papel/modelo e commit analisado. Cada mensagem começa com `[REVISORA]`, `[CONDUTORA]` ou `[EXECUTORA]`. A autora responde na conversa e a revisora confirma o desfecho; sem consenso, a condutora entrega a Douglas as duas posições, evidências, consequências, recomendações e a pergunta decisória. O PR continua aguardando Douglas.
