...
A entrega documental 019 foi integrada pelo PR #26. A execução está na branch `codex/019-user-import-app`, [PR #27](https://github.com/douglasfeitosag/Umanni/pull/27), e permanece no milestone aberto `0.4.0` #7. Ela implementa exclusivamente a importação administrativa CSV/XLSX assíncrona, progresso persistido/ao vivo e configuração local, única e concorrente-segura da senha inicial para contas importadas sem credencial. A numeração substitui a reserva histórica `015-user-import`, já consumida pelo registro pós-0.3.0; não altera decisões de produto.

O candidato fixa `solid_queue 1.7.0` e `roo 3.0.0`, usa as tabelas Solid Queue como migrations do banco `primary`, a fila única `imports`, processo `worker` dedicado e Active Storage em `/rails/storage` compartilhado entre web/worker. O lote, attachment e job são confirmados ou revertidos na mesma transação; o relatório seguro por linha, a retomada idempotente, a invalidação privada via Cable e a agregação de métricas por blocos estão cobertos por testes. Após oito achados da primeira revisão independente, o HEAD de código `6d7d8b45253c1e1091a2d4d36d378f423682bed9` passou novamente `bin/check` e `bin/check-delivery`; ainda faltam a publicação do HEAD documental final, a re-revisão obrigatória, a integração e a transição serial de release. Ver [[EXEC-019-USER-IMPORT]].
