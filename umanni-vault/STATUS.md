...
A execução está publicada no PR #17, branch `codex/013-identity-access-app`. Cadastro, sessão, perfil, CRUD administrativo, invariante concorrente do último administrador, Active Storage, bootstrap, métricas/Cable e interface responsiva foram implementados por ciclos RED/GREEN. Douglas autorizou a expansão mínima da allowlist para o `Dockerfile`, exclusivamente para empacotar channels, queries, services e tasks indispensáveis, e depois autorizou a inclusão no `.dockerignore` dos três assets existentes necessários ao Vite (logo horizontal, Montserrat e Roboto). O gate limpo sem cache do HEAD corretivo histórico passou: Ruby 92,48% de linhas, TypeScript 93,96% de linhas, lint/Brakeman sem achados, Vitest 14/14 e 60/60 Playwright nos seis perfis. A revisão Luna high em contexto separado encontrou e resolveu cinco falhas de acessibilidade/contrato (papel, IDs de diálogos, policy visível, foco de rota e alvos móveis) naquele HEAD. Correções posteriores preservaram esse escopo: espaçamento do hint de avatar, conector de erros em pt-BR, centralização/estado do diálogo destrutivo e fechamento pelo backdrop; o histórico, as evidências e os SHAs correspondentes estão no EXEC-013. Para evitar transferir aceite entre commits, o estado atual de `foundation-checks`, `review-ledger`, labels, threads e `headRefOid` deve ser lido diretamente no PR #17 antes de qualquer merge. Nenhum merge, tag, release ou fechamento do milestone 0.3.0 está autorizado.
