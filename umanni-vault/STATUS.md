...
D-019–D-026 permanecem fechadas. A spec 013 acrescenta D-027–D-031 somente para fechar política de senha, unicidade de e-mail, protocolo de invalidação das métricas, concorrência do último administrador e serialização do bootstrap, cada uma comparando três cenários. A revisora independente `gpt-5.6-luna` high aceitou o planejamento no HEAD `74ef24610333751723e4613e88852613244b043a` depois de sete achados corrigidos e resolvidos. O prompt da futura executora foi criado somente após esse aceite; como sua inclusão gera novo commit, o novo HEAD precisa de revalidação externa no PR antes da entrega. Implementação, merge, tag e release ainda não estão autorizados.

## Pendências e momento de resolução

- O roadmap documental 012 organiza B001–B007 sem executar aplicação e foi integrado pelo PR #15. A spec detalhada 013 de identidade/acesso para 0.3.0 está no PR #16 aguardando revisão independente; importação CSV/XLSX permanece para 0.4.0; CI/runner, extras e issue #9 preservam destino Backlog.
