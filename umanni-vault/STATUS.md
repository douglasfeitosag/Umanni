...
## Credenciais Rails 1.1.0 — candidata em implementação

A entrega 023 parte do HEAD de planejamento `9e4744adea9deac7637b7b95d633aced7921c0a5`, já aceito pela revisão independente com `review-ledger=success`, `spec-reviewed` e zero threads. Ela é deliberadamente limitada a mover o segredo de aplicação de `SECRET_KEY_BASE` para `config/credentials.yml.enc`, passando `RAILS_MASTER_KEY` apenas em runtime aos processos delivery. URLs de banco e seus gates de isolamento continuam em Compose/ambiente.

O ciclo RED observou seis falhas do contrato plaintext; o GREEN focado passou 7 exemplos. O build production sem master key passou usando somente `SECRET_KEY_BASE_DUMMY=1`; a imagem auditada continha ciphertext e não continha `config/master.key`, `.env`, specs, Dockerfile, Compose ou caches. Com a chave privada local, web e worker ficaram saudáveis e `/ready`/`/up` responderam pela porta isolada 3131, pois a porta pública 3030 estava ocupada por recurso externo. Sem chave, o web encerrou com status 1 e a mensagem segura `delivery.startup.credentials_missing: supply RAILS_MASTER_KEY`.
