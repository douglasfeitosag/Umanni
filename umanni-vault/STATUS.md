...
## Importação de usuários 0.4.0 — candidata à revisão

A entrega documental 019 foi integrada pelo PR #26. A execução está na branch `codex/019-user-import-app`, [PR #27](https://github.com/douglasfeitosag/Umanni/pull/27), e permanece no milestone aberto `0.4.0` #7. Ela implementa exclusivamente a importação administrativa CSV/XLSX assíncrona, progresso persistido/ao vivo e configuração local, única e concorrente-segura da senha inicial para contas importadas sem credencial. Em 2026-09-16, Douglas autorizou pontualmente a tela administrativa já existente de edição e os testes Playwright correspondentes, exclusivamente para completar essa configuração de senha; nenhuma outra exclusão de escopo foi alterada. A numeração substitui a reserva histórica `015-user-import`, já consumida pelo registro pós-0.3.0; não altera decisões de produto.

O candidato fixa `solid_queue 1.7.0` e `roo 3.0.0`, usa as tabelas Solid Queue como migrations do banco `primary`, a fila única `imports`, processo `worker` dedicado e Active Storage em `/rails/storage` compartilhado entre web/worker. O lote, attachment e job são confirmados ou revertidos na mesma transação; o relatório seguro por linha, a retomada idempotente, a invalidação privada via Cable e a agregação de métricas por blocos estão cobertos por testes. Após doze achados em duas rodadas de revisão, o HEAD de código `02da120ce69f9d3fc9bfb752e92c26baad17c5d9` passou `bin/check` com 92 RSpec, 23 Vitest e 66 Playwright; ainda faltam a confirmação do gate de entrega, a publicação do HEAD documental final, a re-revisão obrigatória, a integração e a transição serial de release. Ver [[EXEC-019-USER-IMPORT]].
...
- A candidata CSV/XLSX 0.4.0 está tecnicamente validada no HEAD registrado em [[EXEC-019-USER-IMPORT]], mas não está aceita, integrada ou publicada; CI/runner, extras e issue #9 preservam destino Backlog.
...
- Antes da integração 019: publicar o HEAD documental final no PR #27, obter revisão independente Luna/high, `review-ledger=success`, `code-reviewed` e zero threads abertas. Os demais fluxos de identidade/acesso já pertencem à 0.3.0 publicada.
