...
- `normalized_email`: opcional, já aparado/minúsculo e limitado;
...
Repetição automática pode entrar novamente no executor a partir de `queued`, `processing` ou `failed`, mas não regride o estado persistido nem reprocessa `UserImportRow` terminal. Estados `completed` e `completed_with_errors` são no-op para o mesmo job.
...
7. Atualizar contadores persistidos e emitir invalidação após commit por bloco.
