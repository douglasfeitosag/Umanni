...
Parar sem criar/mover tag se o HEAD/base mudar sem revisão, se `v0.1.0` ou a release aparecerem previamente, se houver item de `0.1.0` aberto sem decisão, thread bloqueante, ledger inválido ou conflito. A entrega só termina quando tag anotada e release final existirem, apontarem ao commit integrado, milestone `0.1.0` estiver fechado, issue 9 permanecer em `Backlog` e a evidência estiver publicada no PR.
