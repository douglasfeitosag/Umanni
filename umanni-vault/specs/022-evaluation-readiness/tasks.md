...
- [ ] T019a Use the already recorded repeated full-matrix 65/66 auxiliary-administrator login failure as RED evidence. Add the minimal test-only persistence observation in `spec/e2e/identity.spec.ts` before each independent auxiliary-admin login, retain all browser/viewport concurrency and `retries: 0`, then prove GREEN with one new full six-project 66-scenario matrix. This post-GREEN execution is required and is not a per-scenario retry.
- [ ] T019 After T019a GREEN, run `git diff --check`, the redacted `.env`/private-guide tracking checks from T006, `bin/check`, and `bin/check-delivery` once on the candidate HEAD; record only actual results. Stop on any unrelated failure; the known auxiliary-administrator race is handled only by T019a.
