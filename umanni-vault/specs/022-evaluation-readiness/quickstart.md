...
4. Run the bootstrap command with fresh local-only values and `UMANNI_BOOTSTRAP_LOCAL_DELIVERY=1`. Confirm that production rejects the same operation without the opt-in, then repeat the opted-in command and confirm the existing administrator is unchanged without printing the password.
