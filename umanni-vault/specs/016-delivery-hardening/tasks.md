...
- [x] **P007 — validação** Executar análise estrutural/consistência, links, Markdown e `git diff --check`; corrigir somente documentação.
- [x] **P008 — publicação** Criar commits documentais coesos, push e PR com milestone 0.3.1, label `documentation` e Douglas responsável.
...
- [ ] **E004 — readiness RED/GREEN** Provar que a própria resposta HTTP demonstra o Rails atendendo, consulta real/migrations prontas, 503 constante sem detalhe, ausência de GET interno a `/up` e `/up` intacto; implementar endpoint/controlador isolado e apontar somente healthcheck `delivery`.
...
- [ ] **E006 — exceções RED** Adicionar fixture/rota para requests no ambiente de teste e preparar overlay/initializer read-only sob `spec/support/` para injetar o mesmo probe na imagem final com `RAILS_ENV=production`; criar specs falhando para HTML/Inertia 5xx, status, MIME inválido, ausência de shares/segredos e regressão 403/404/422.
...
- [ ] **E009 — acessibilidade/sistema** Executar Playwright dedicado contra o container production-like com probe montado somente pelo overlay; cobrir HTML/Inertia, três engines, desktop/mobile, teclado, 200%, reduced motion, 44×44 e overflow; garantir que não aparece modal de resposta inválida.
- [ ] **E010 — segurança** Executar sentinelas em exceção/params/cookie/URL/ambiente; provar ausência em body, headers e logs, igualdade dos eventos próprios com a allowlist constante e ausência do arquivo/rota de probe na imagem sem overlay.
...
| US1.3 falha fechada | exit code, porta indisponível, eventos na allowlist constante | E003/E005/E010 |
| US2.1 pronto | request readiness sem GET interno + health Compose via HTTP | E004/E005 |
...
| US3.2 Inertia 5xx | request headers/page object + Playwright production-like sem modal | E006–E009 |
