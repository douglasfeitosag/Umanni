...
if release_probe=$(gh api repos/douglasfeitosag/Umanni/releases/tags/v0.2.1 2>&1); then
  printf '%s\n' 'ERRO: release v0.2.1 já existe.' >&2
  exit 1
fi
...
```sh
if rg -n -i --glob '!**/011-spec-state-reconciliation/quickstart.md' 'awaiting Douglas.s integration|Ainda não há aplicação|0\.2\.0 ainda é planejamento|release preparation in progress|Rascunho para revisão|handoff pós-merge em revisão' README.md AGENTS.md umanni-vault/STATUS.md umanni-vault/MEMORIA-PROJETO.md umanni-vault/specs; then
  printf '%s\n' 'ERRO: claim vigente obsoleto encontrado.' >&2
  exit 1
fi
```
